...
export async function POST(
  ...
  if (!student.senha) {
    return NextResponse.json(
      { message: "Seu acesso ainda não foi configurado. Fale com seu personal! 😊" },
      { status: 403 }
    );
  }
  ...
  if (student.senha !== hashed) {
    return NextResponse.json(
      { message: "Senha incorreta 💪 Verifique com seu personal trainer!" },
      { status: 401 }
    );
  }
  ...
  return NextResponse.json({
    ok: true,
    student: { id: student.id, nome: student.nome, userId: student.userId },
  });
}
