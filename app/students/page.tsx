    ...
    <div className="modal-backdrop fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="modal-content bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-dark-600">
          ...
          <button onClick={onClose} className="text-dark-200 hover:text-white transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                value={form.biotipo}
                onChange={(e) => setForm((f) => ({ ...f, biotipo: e.target.value as (typeof BIOTIPOS)[number] }))}
              >
                {BIOTIPOS.map((b) => (
...
function WeightChart({ student }: { student: Student }) {
  const data = (student.historicoPeso || [{ data: student.createdAt, peso: student.peso }]).map((h) => ({
