...
import {
  ...
  FileJson, FileText, FileSpreadsheet, FileType2, Eye
} from 'lucide-react'
...
function WorkoutModal({ students, userId, workout, onClose, onSave }: WorkoutModalProps) {
  ...
  const updateDiaDia = (id: string, diaSemana: string) =>
    setDias(d => d.map(x => x.id === id ? { ...x, diaSemana } : x))
  ...
  const handleSave = () => {
    ...
    if (workout) {
      updateWorkout({ ...workout, studentId, objetivo, nivel, dias })
    } else {
      createWorkout(userId, { studentId, objetivo, nivel, dias })
    }
    onSave()
    onClose()
  }
                  ...
                  <div className="max-h-44 overflow-y-auto">
                    {filteredStudents.map(s => (
                      <button
                        key={s.id}
                        type="button"
                        onClick={() => { setStudentId(s.id); setShowStudentDD(false); setStudentSearch('') }}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-dark-300">{s.biotipo} · {s.peso}kg</p>
                        </div>
                      </button>
                    ))}
                ...
                <div key={dia.id} className="bg-dark-700/50 border border-dark-600 rounded-xl overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 bg-dark-700 border-b border-dark-600">
                    <div className="relative">
                      ...
                      >
                        {DIAS.map(d => <option key={d} className="bg-dark-800 text-white">{d}</option>)}
                      </select>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-dark-300">{dia.exercises.length} exercício{dia.exercises.length !== 1 ? 's' : ''}</span>
...
export default function WorkoutsPage() {
  ...
  const filtered = workouts.filter(w => {
    ...
    return !search || student?.nome.toLowerCase().includes(search.toLowerCase()) ||
      w.objetivo.toLowerCase().includes(search.toLowerCase())
  })

  const handleDelete = (id: string) => {
    if (confirm('Excluir este treino?')) { deleteWorkout(id); load() }
  }
                ...
                <div key={workout.id} className="card p-5 hover:border-dark-500 transition-colors">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setViewing(workout)} className="w-8 h-8 rounded-lg bg-dark-600 hover:bg-dark-500 flex items-center justify-center text-dark-200 hover:text-white transition-all">
                        <Eye size={13} />
                      </button>
                      <button onClick={() => { setEditing(workout); setModalOpen(true) }} className="w-8 h-8 rounded-lg bg-dark-600 hover:bg-dark-500 flex items-center justify-center text-dark-200 hover:text-white transition-all">
                        <Edit2 size={13} />
                      </button>
                      <button onClick={() => handleDelete(workout.id)} className="w-8 h-8 rounded-lg bg-dark-600 hover:bg-red-500/10 flex items-center justify-center text-dark-200 hover:text-red-400 transition-all">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
